...
export function FileUpload({ 
  ...
  const handleFiles = async (newFiles: File[]) => {
    ...
    try {
      ...
      const uploadedResults = data.media.map((serverMedia: any, index: number) => {
        return {
          id: Math.random().toString(36).substring(7),
          file: validFiles[index],
          previewUrl: serverMedia.url,
          publicId: serverMedia.publicId,
          type: serverMedia.type
        } as UploadedFile;
      });
