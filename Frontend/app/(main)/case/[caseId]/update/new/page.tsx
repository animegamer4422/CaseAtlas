...
export default function NewUpdatePage() {
  ...
  // type choices could map to tags, but backend currently just takes content. We'll prepend type to title if needed, or ignore.
  ...
  useEffect(() => {
    api.get(`/cases/${caseId}`).then(setCaseItem).catch(console.error);
  }, [caseId]);
  ...
  const handlePublish = async () => {
    ...
    try {
      const mediaToSend = mediaFiles.map(m => ({
        url: m.previewUrl,
        publicId: m.publicId,
        type: m.type
      }));

      await api.post(`/cases/${caseId}/updates`, {
        title: title.trim(),
        content: body.trim(),
        media: mediaToSend,
      });
      ...
      setPublished(true);
      setTimeout(() => router.push(`/case/${caseItem.caseId}`), 1800);
    } catch (e) {
      console.error(e);
      // add toast
    } finally {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '60vh', gap: 16, animation: 'fadeInUp 0.4s ease' }}>
        ...
        <p style={{ color: 'var(--text-muted)', textAlign: 'center', maxWidth: 320 }}>
          Subscribers to this case will be notified if approved.
        </p>
      </div>
    );
  }
    ...
    <div style={{ padding: '24px 32px 48px', maxWidth: 700, position: 'relative', zIndex: 1 }}>
      ...
      <div style={{ marginBottom: 32 }}>
        ...
        <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem' }}>
          for <span style={{ color: 'var(--indigo-300)', fontFamily: 'JetBrains Mono, monospace', fontWeight: 500 }}>{caseItem.caseId}</span>
          {' — '}{caseItem.title}
        </p>
      </div>
      ...
      <div className="input-group" style={{ marginBottom: 28 }}>
        ...
        <FileUpload onUpload={setMediaFiles} endpoint="/upload/update-media" label="Attach Evidence (Photos/Videos)" />
      </div>
      ...
      <div style={{
        ...
        <div>
          <p style={{ fontSize: '0.8125rem', fontWeight: 600, color: 'var(--amber-400)', marginBottom: 2 }}>Publishing Notice</p>
          <p style={{ fontSize: '0.8125rem', color: 'var(--text-muted)', lineHeight: 1.6 }}>
            If you are a moderator, this will be marked <strong style={{ color: 'var(--green-400)' }}>Approved</strong> immediately and all <strong style={{ color: 'var(--text-secondary)' }}>{caseItem.subscribers?.length || 0}</strong> subscribers will receive a notification.
          </p>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
        ...
        <Link href={`/case/${caseItem.caseId}`} className="btn btn-ghost">
          Cancel
        </Link>
      </div>
