'use client';

import { useState, useEffect } from 'react';
import { useParams, notFound } from 'next/navigation';
import Link from 'next/link';
import {
  Bell, BellOff, Share2, Copy, Check, MapPin, Calendar,
  Tag, Users, MessageSquare, FileText, Shield, ChevronLeft,
  Clock, Plus, ExternalLink, Flag, ThumbsUp, Send,
  AlertTriangle, CheckCircle, Archive, Edit3, Zap,
  Newspaper, User as UserIcon, Link2, Paperclip, X
} from 'lucide-react';
import { getCategoryLabel, formatRelativeTime } from '@/lib/mockData';
import { api } from '@/lib/api';
import { useAuth } from '@/components/providers/AuthProvider';
import io from 'socket.io-client';

const TABS = ['Overview', 'Updates', 'Discussion'] as const;
type Tab = typeof TABS[number];

export default function CasePage() {
  const params = useParams();
  const caseId = params.caseId as string;
  const { user } = useAuth();

  const [caseItem, setCaseItem] = useState<any>(null);
  const [isLoading, setIsLoading] = useState(true);

  const [activeTab, setActiveTab] = useState<Tab>('Overview');
  const [subscribed, setSubscribed] = useState(false);
  const [copied, setCopied] = useState(false);
  const [newComment, setNewComment] = useState('');
  const [comments, setComments] = useState<any[]>([]);
  const [updates, setUpdates] = useState<any[]>([]);
  
  const [commentMediaFiles, setCommentMediaFiles] = useState<File[]>([]);
  const [isUploading, setIsUploading] = useState(false);
  const [uploadProgress, setUploadProgress] = useState(0);
  const [activeMedia, setActiveMedia] = useState<any>(null);

  // Esc key to close lightbox
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'Escape') setActiveMedia(null);
    };
    if (activeMedia) window.addEventListener('keydown', handleKeyDown);
    return () => window.removeEventListener('keydown', handleKeyDown);
  }, [activeMedia]);

  useEffect(() => {
    let socket: any;

    const fetchData = async () => {
      try {
        const c = await api.get(`/cases/${caseId}`);
        setCaseItem(c);

        if (user) {
          const subs = await api.get('/users/me/subscriptions');
          const isSub = subs.some((s: any) => s.caseId._id === c._id || s.caseId.caseId === c.caseId);
          setSubscribed(isSub);
        }

        const [updatesData, commentsData] = await Promise.all([
          api.get(`/cases/${caseId}/updates`),
          api.get(`/cases/${caseId}/comments`),
        ]);

        setUpdates(updatesData);
        setComments(commentsData);

        // Realtime
        const socketUrl = process.env.NEXT_PUBLIC_SOCKET_URL || 'http://localhost:5000';
        socket = io(socketUrl, {
           query: user ? { userId: user._id } : {}
        });

        socket.on('connect', () => {
          socket.emit('join_case', caseId);
        });

        socket.on('new_comment', (comment: any) => {
          setComments((prev: any[]) => {
            if (prev.some((c: any) => c._id === comment._id)) return prev;
            return [comment, ...prev];
          });
        });

        socket.on('case_update', () => {
          // Refresh updates
          api.get(`/cases/${caseId}/updates`).then(setUpdates);
        });

      } catch (error) {
        console.error('Fetch error:', error);
      } finally {
        setIsLoading(false);
      }
    };
    fetchData();

    return () => {
      if (socket) socket.disconnect();
    };
  }, [caseId, user]);

  if (isLoading) {
    return <div style={{ padding: 40, textAlign: 'center', color: 'var(--text-muted)' }}>Loading case...</div>;
  }

  if (!caseItem) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '60vh', gap: 16 }}>
        <div className="empty-icon"><AlertTriangle size={24} /></div>
        <h2>Case not found</h2>
        <p>No case with ID <span className="mono" style={{ color: 'var(--indigo-300)' }}>{caseId}</span> exists.</p>
        <Link href="/" className="btn btn-primary">← Back to Home</Link>
      </div>
    );
  }

  const isMod = user && (user.role === 'admin' || caseItem.moderators.some((m: any) => m._id === user._id));
  const creator = caseItem.createdBy;

  const copyLink = () => {
    navigator.clipboard.writeText(window.location.href);
    setCopied(true);
    setTimeout(() => setCopied(false), 2500);
  };

  const handleSubscribe = async () => {
    if (!user) return alert("Please login to subscribe");
    try {
      if (subscribed) {
        await api.delete(`/cases/${caseId}/subscribe`);
        setSubscribed(false);
        setCaseItem((prev: any) => ({...prev, subscribers: prev.subscribers.filter((s: any) => s._id !== user._id)}));
        window.dispatchEvent(new Event('subscription_updated'));
      } else {
        await api.post(`/cases/${caseId}/subscribe`, {});
        setSubscribed(true);
        setCaseItem((prev: any) => ({...prev, subscribers: [...prev.subscribers, { _id: user._id }]}));
        window.dispatchEvent(new Event('subscription_updated'));
      }
    } catch (e) {
      console.error(e);
    }
  };

  const submitComment = async () => {
    if ((!newComment.trim() && commentMediaFiles.length === 0) || !user || isUploading) return;
    setIsUploading(true);
    setUploadProgress(0); // reset
    try {
      let finalMedia = [];
      if (commentMediaFiles.length > 0) {
        // Pseudo progress simulation since fetch lacks native progress
        const simProgress = setInterval(() => {
          setUploadProgress(p => p < 90 ? p + 10 : p);
        }, 150);

        const formData = new FormData();
        commentMediaFiles.forEach((file) => formData.append('media', file));
        const res = await api.post('/upload/comment-media', formData);
        
        clearInterval(simProgress);
        setUploadProgress(100);
        finalMedia = res.media || [];
      }
      const newSavedComment = await api.post(`/cases/${caseId}/comments`, { content: newComment.trim(), media: finalMedia });
      setNewComment('');
      setCommentMediaFiles([]);
      
      setComments((prev: any[]) => {
        if (prev.some((c: any) => c._id === newSavedComment._id)) return prev;
        return [newSavedComment, ...prev];
      });
    } catch (e) {
      console.error('Failed to post comment', e);
    } finally {
      setIsUploading(false);
    }
  };

  const statusBadge: Record<string, string> = {
    open: 'badge-open',
    resolved: 'badge-resolved',
    unknown: 'badge-unknown',
    archived: 'badge-archived',
  };

  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', position: 'relative', zIndex: 1 }}>
      {/* ── Sticky header + tabs ── */}
      <div style={{
        padding: '24px 32px 0',
        borderBottom: '1px solid var(--border-subtle)',
        background: 'rgba(10,10,18,0.5)',
        backdropFilter: 'blur(12px)',
        flexShrink: 0,
      }}>
        <Link
          href="/"
          style={{ display: 'inline-flex', alignItems: 'center', gap: 6, color: 'var(--text-muted)', fontSize: '0.8125rem', textDecoration: 'none', marginBottom: 16, transition: 'color 0.2s' }}
        >
          <ChevronLeft size={14} /> All cases
        </Link>

        <div style={{ display: 'flex', gap: 20, alignItems: 'flex-start', marginBottom: 20 }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10, flexWrap: 'wrap' }}>
              <span className={`badge ${statusBadge[caseItem.status]} badge-dot`}>
                {caseItem.status}
              </span>
              <span className="badge" style={{ background: 'rgba(99,102,241,0.08)', color: 'var(--text-muted)', border: '1px solid var(--border-subtle)' }}>
                {getCategoryLabel(caseItem.category)}
              </span>
              {isMod && (
                <span className="badge" style={{ background: 'rgba(245,158,11,0.10)', color: 'var(--amber-400)', border: '1px solid rgba(245,158,11,0.20)' }}>
                  <Shield size={10} /> Moderator
                </span>
              )}
            </div>
            <h1 style={{ fontSize: 'clamp(1.25rem, 3vw, 1.75rem)', fontWeight: 800, letterSpacing: '-0.025em', marginBottom: 10, lineHeight: 1.25, color: 'var(--text-primary)' }}>
              {caseItem.title}
            </h1>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16, fontSize: '0.8125rem', color: 'var(--text-muted)' }}>
              <span style={{ display: 'flex', alignItems: 'center', gap: 5 }}><MapPin size={13} /> {caseItem.location || 'Unknown'}</span>
              <span style={{ display: 'flex', alignItems: 'center', gap: 5 }}><Calendar size={13} /> {formatRelativeTime(caseItem.createdAt)}</span>
              <span style={{ display: 'flex', alignItems: 'center', gap: 5 }}><Users size={13} /> {caseItem.subscribers?.length || 0} subscribers</span>
              <span style={{ display: 'flex', alignItems: 'center', gap: 5 }}><Clock size={13} /> Updated {formatRelativeTime(caseItem.updatedAt)}</span>
            </div>
          </div>

          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, alignItems: 'flex-end', flexShrink: 0 }}>
            <button className="case-id-chip" onClick={copyLink} title="Copy case link">
              {copied ? <Check size={13} /> : <Copy size={13} />}
              {caseItem.caseId}
            </button>

            <div style={{ display: 'flex', gap: 8 }}>
              <button
                className={`btn ${subscribed ? 'btn-secondary' : 'btn-primary'}`}
                onClick={handleSubscribe}
                style={{ gap: 7 }}
              >
                {subscribed ? <BellOff size={15} /> : <Bell size={15} />}
                {subscribed ? 'Subscribed' : 'Subscribe'}
              </button>
              <button className="btn btn-secondary btn-icon" onClick={copyLink} title="Share">
                {copied ? <Check size={16} /> : <Share2 size={16} />}
              </button>
            </div>

            {isMod && (
              <Link href={`/case/${caseItem.caseId}/update/new`} className="btn btn-amber" style={{ fontSize: '0.8125rem' }}>
                <Plus size={14} /> Post Official Update
              </Link>
            )}
          </div>
        </div>

        <div style={{ display: 'flex', gap: 2, borderBottom: 'none', marginBottom: -1 }}>
          {TABS.map(tab => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              style={{
                display: 'flex', alignItems: 'center', gap: 7,
                padding: '10px 16px',
                fontSize: '0.875rem',
                fontWeight: activeTab === tab ? 600 : 400,
                color: activeTab === tab ? 'var(--indigo-300)' : 'var(--text-muted)',
                background: 'none',
                border: 'none',
                borderBottom: activeTab === tab ? '2px solid var(--indigo-500)' : '2px solid transparent',
                cursor: 'pointer',
                transition: 'all 0.2s',
                whiteSpace: 'nowrap',
              }}
            >
              {tabIcon(tab)}
              {tab}
              {tab === 'Updates' && <span style={{ fontSize: '0.75rem', padding: '1px 6px', borderRadius: 'var(--radius-full)', background: 'rgba(99,102,241,0.10)', color: 'var(--indigo-300)' }}>{updates.filter(u=>u.isApproved).length}</span>}
              {tab === 'Discussion' && <span style={{ fontSize: '0.75rem', padding: '1px 6px', borderRadius: 'var(--radius-full)', background: 'rgba(99,102,241,0.10)', color: 'var(--indigo-300)' }}>{comments.length}</span>}
            </button>
          ))}
        </div>
      </div>

      {/* ── Scrollable tab content panel ── */}
      <div style={{ flex: 1, minHeight: 0, overflowY: 'auto' }}>
        <div style={{ padding: '28px 32px 48px', maxWidth: 860 }}>
        {activeTab === 'Overview' && (
          <div className="animate-in" style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
            <div className="stats-row">
              <div className="stat-box">
                <div className="stat-box-value">{caseItem.updateCount || 0}</div>
                <div className="stat-box-label">Updates</div>
              </div>
              <div className="stat-box">
                <div className="stat-box-value">{caseItem.subscribers?.length || 0}</div>
                <div className="stat-box-label">Subscribers</div>
              </div>
              <div className="stat-box">
                <div className="stat-box-value">{caseItem.commentCount || 0}</div>
                <div className="stat-box-label">Comments</div>
              </div>
              <div className="stat-box">
                <div className="stat-box-value">{caseItem.media?.length || 0}</div>
                <div className="stat-box-label">Media</div>
              </div>
            </div>

            <div className="card" style={{ padding: 24 }}>
              <h3 style={{ color: 'var(--text-primary)', marginBottom: 12, display: 'flex', alignItems: 'center', gap: 8 }}>
                <FileText size={16} style={{ color: 'var(--indigo-400)' }} />
                Summary
              </h3>
              <p style={{ fontSize: '0.9375rem', lineHeight: 1.75, color: 'var(--text-secondary)' }}>{caseItem.description}</p>

              {caseItem.media && caseItem.media.length > 0 && (
                <div style={{ marginTop: 24, paddingTop: 20, borderTop: '1px solid var(--border-subtle)' }}>
                  <h4 style={{ fontSize: '0.8125rem', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 12 }}>Initial Evidence</h4>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 12 }}>
                    {caseItem.media.map((m: any) => (
                      <div key={m._id} style={{ borderRadius: 'var(--radius-md)', overflow: 'hidden', border: '1px solid var(--border-subtle)', background: 'var(--bg-elevated)', aspectRatio: '16/9' }}>
                        {m.type === 'image' && (
                          <img src={m.url} alt="attached media" style={{ width: '100%', height: '100%', objectFit: 'cover' }} loading="lazy" />
                        )}
                        {m.type === 'video' && (
                          <video src={m.url} controls style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                        )}
                        {m.type === 'document' && (
                          <a href={m.url} target="_blank" rel="noopener noreferrer" style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textDecoration: 'none' }}>
                            <FileText size={24} style={{ color: 'var(--indigo-400)', marginBottom: 8 }} />
                            <span style={{ fontSize: '0.75rem', color: 'var(--text-primary)', fontWeight: 600, textAlign: 'center' }}>View<br/>Document</span>
                          </a>
                        )}
                      </div>
                    ))}
                  </div>
                </div>
              )}
            </div>

            <div className="card" style={{ padding: 24 }}>
              <h3 style={{ color: 'var(--text-primary)', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
                <Zap size={16} style={{ color: 'var(--amber-400)' }} />
                Key Facts
              </h3>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                <FactRow label="Category" value={getCategoryLabel(caseItem.category)} />
                <FactRow label="Location" value={caseItem.location} />
                <FactRow label="Status" value={caseItem.status?.charAt(0).toUpperCase() + caseItem.status?.slice(1)} />
                <FactRow label="Visibility" value={(caseItem.visibility || 'public').charAt(0).toUpperCase() + (caseItem.visibility || 'public').slice(1)} />
                <FactRow label="Case Created" value={formatRelativeTime(caseItem.createdAt)} />
                <FactRow label="Created by" value={creator?.username || 'Unknown'} />
                <FactRow label="Case ID" value={caseItem.caseId} mono />
              </div>
            </div>

            {/* Tags could go here if implemented on backend */}
          </div>
        )}

        {/* ── Updates ── */}
        {activeTab === 'Updates' && (
          <div className="animate-in">
            {isMod && (
              <div style={{ marginBottom: 20 }}>
                <Link href={`/case/${caseItem.caseId}/update/new`} className="btn btn-amber">
                  <Plus size={15} /> Post Official Update
                </Link>
              </div>
            )}

            {updates.length === 0 ? (
              <div className="empty-state">
                <div className="empty-icon"><Bell size={24} /></div>
                <h3>No official updates yet</h3>
                <p>Moderators will post verified updates here. Subscribe to be notified.</p>
              </div>
            ) : (
              <div className="timeline">
                {updates.filter(u => u.isApproved || isMod).map((u, i, arr) => (
                  <UpdateCard key={u._id} update={u} isLast={i === arr.length - 1} isMod={isMod} caseId={caseId} onApprove={() => {
                    api.patch(`/updates/${u._id}/approve`, {}).then(() => {
                      api.get(`/cases/${caseId}/updates`).then(setUpdates);
                    })
                  }} />
                ))}
              </div>
            )}
          </div>
        )}

        {/* ── Discussion ── */}
        {activeTab === 'Discussion' && (
          <div className="animate-in">
            {user ? (
            <div className="card" style={{ padding: 20, marginBottom: 24 }}>
              <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                <div className="avatar">{user.avatarInitials || user.username.substring(0,2).toUpperCase()}</div>
                <div style={{ flex: 1 }}>
                  <textarea
                    className="input"
                    placeholder="Share what you know. Be specific, be civil, cite sources when possible."
                    value={newComment}
                    onChange={e => setNewComment(e.target.value)}
                    style={{ minHeight: 90, resize: 'vertical', marginBottom: 10 }}
                  />
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      <label className="btn btn-ghost btn-sm" style={{ padding: '0 8px', cursor: 'pointer' }}>
                        <Paperclip size={16} style={{ color: 'var(--text-muted)' }} />
                        <input 
                          type="file" multiple accept="image/*,video/*" 
                          style={{ display: 'none' }} 
                          onChange={(e) => setCommentMediaFiles((prev: File[]) => [...prev, ...Array.from(e.target.files || [])])}
                        />
                      </label>
                      <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>
                        Be accurate. Avoid assumptions without evidence.
                      </span>
                    </div>
                    <button
                      className="btn btn-primary btn-sm"
                      onClick={submitComment}
                      disabled={(!newComment.trim() && commentMediaFiles.length === 0) || isUploading}
                      style={{ gap: 6, opacity: isUploading ? 0.7 : 1, position: 'relative', overflow: 'hidden' }}
                    >
                      {/* Progress bar background overlay */}
                      {isUploading && (
                        <div style={{ position: 'absolute', top: 0, left: 0, height: '100%', background: 'rgba(255,255,255,0.2)', width: `${uploadProgress}%`, transition: 'width 0.2s' }} />
                      )}
                      {isUploading ? <span className="spinner" style={{width: 13, height: 13, border: '2px solid rgba(255,255,255,0.3)', borderTopColor: 'white', borderRadius: '50%', animation: 'spin 1s linear infinite', position: 'relative', zIndex: 1}} /> : <Send size={13} style={{ position: 'relative', zIndex: 1}} />}
                      <span style={{ position: 'relative', zIndex: 1 }}>{isUploading ? (uploadProgress < 100 ? `Uploading ${uploadProgress}%` : 'Posting...') : 'Post'}</span>
                    </button>
                  </div>
                  {commentMediaFiles.length > 0 && (
                    <div style={{ display: 'flex', gap: 8, marginTop: 12, flexWrap: 'wrap' }}>
                      {commentMediaFiles.map((file, i) => (
                        <div key={i} style={{ padding: '4px 8px', background: 'var(--bg-elevated)', borderRadius: 'var(--radius-sm)', fontSize: '0.75rem', display: 'flex', alignItems: 'center', gap: 6 }}>
                          <span style={{ maxWidth: 100, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{file.name}</span>
                          <X size={12} style={{cursor: 'pointer'}} onClick={() => setCommentMediaFiles((prev: File[]) => prev.filter((_, idx) => idx !== i))} />
                        </div>
                      ))}
                    </div>
                  )}
                </div>
              </div>
            </div>
            ) : (
              <div className="card" style={{ padding: 20, marginBottom: 24, textAlign: 'center' }}>
                <p style={{ color: 'var(--text-muted)' }}>Log in to join the discussion.</p>
                <Link href="/login" className="btn btn-primary btn-sm" style={{ marginTop: 10 }}>Log in</Link>
              </div>
            )}

            {comments.length === 0 ? (
              <div className="empty-state">
                <div className="empty-icon"><MessageSquare size={24} /></div>
                <h3>No comments yet</h3>
                <p>Be the first to share relevant information or context.</p>
              </div>
            ) : (
              <div>
                {comments.map(comment => (
                  <CommentItem key={comment._id} comment={comment} onMediaClick={setActiveMedia} />
                ))}
              </div>
            )}
          </div>
        )}

      </div>  {/* end padding div */}
      </div>  {/* end scrollable panel */}

      {/* Lightbox Media Viewer overlay – click backdrop to close */}
      {activeMedia && (
        <div 
          onClick={() => setActiveMedia(null)}
          style={{
            position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh',
            background: 'rgba(0,0,0,0.88)', backdropFilter: 'blur(6px)',
            zIndex: 99999, display: 'flex', alignItems: 'center', justifyContent: 'center',
            padding: 40, cursor: 'zoom-out'
          }}
        >
          {/* Close button – fixed top-right */}
          <button 
            onClick={() => setActiveMedia(null)}
            style={{
              position: 'fixed', top: 20, right: 20, width: 44, height: 44,
              background: 'rgba(255,255,255,0.1)', backdropFilter: 'blur(10px)', color: 'white', borderRadius: '50%',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              border: '1px solid rgba(255,255,255,0.2)', cursor: 'pointer', zIndex: 999999
            }}
          >
            <X size={24} />
          </button>

          {/* Media – stopPropagation keeps clicks on the media from closing */}
          <div onClick={(e) => e.stopPropagation()} style={{ cursor: 'default', display: 'flex', alignItems: 'center', justifyContent: 'center', maxWidth: '90vw', maxHeight: '90vh' }}>
            {activeMedia.type === 'image' && (
              <img src={activeMedia.url} alt="" style={{ maxWidth: '100%', maxHeight: '90vh', objectFit: 'contain', borderRadius: 8, boxShadow: '0 0 80px rgba(0,0,0,0.8)' }} />
            )}
            {activeMedia.type === 'video' && (
              <video src={activeMedia.url} style={{ maxWidth: '100%', maxHeight: '90vh', objectFit: 'contain', borderRadius: 8, boxShadow: '0 0 80px rgba(0,0,0,0.8)' }} controls autoPlay />
            )}
          </div>
        </div>
      )}

    </div>
  );
}

// ─── Sub-components ───────────────────────────────────────────────────────────

function tabIcon(tab: Tab) {
  const icons = {
    Overview: <FileText size={14} />,
    Updates: <Bell size={14} />,
    Discussion: <MessageSquare size={14} />,
  };
  return icons[tab];
}

function FactRow({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
      <span style={{ fontSize: '0.6875rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--text-muted)' }}>
        {label}
      </span>
      <span style={{
        fontSize: '0.875rem',
        fontFamily: mono ? 'JetBrains Mono, monospace' : 'Inter, sans-serif',
        fontWeight: mono ? 500 : 400,
        color: mono ? 'var(--indigo-300)' : 'var(--text-primary)',
      }}>
        {value}
      </span>
    </div>
  );
}

function UpdateCard({ update, isLast, isMod, onApprove }: { update: any; isLast: boolean, isMod: boolean, caseId: string, onApprove: ()=>void }) {
  const author = update.postedBy;

  return (
    <div className="timeline-item" style={{ animationFillMode: 'both' }}>
      {!isLast && <div className="timeline-line" />}
      <div className="timeline-dot">
        <CheckCircle size={14} style={{ color: update.isApproved ? 'var(--green-400)' : 'var(--amber-400)' }} />
      </div>
      <div className="timeline-content">
        <div className="card" style={{ padding: 20, border: update.isApproved ? '' : '1px solid var(--amber-400)' }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, marginBottom: 12 }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <span style={{
                display: 'inline-flex', alignItems: 'center', gap: 5,
                padding: '2px 10px', borderRadius: 'var(--radius-full)',
                background: update.isApproved ? 'rgba(74,222,128,0.10)' : 'rgba(245,158,11,0.10)', color: update.isApproved ? 'var(--green-400)' : 'var(--amber-400)',
                fontSize: '0.6875rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em',
                width: 'fit-content',
              }}>
                <CheckCircle size={10} /> {update.isApproved ? 'Official Update' : 'Pending Approval'}
              </span>
              <h3 style={{ fontSize: '1rem', color: 'var(--text-primary)', fontWeight: 700, lineHeight: 1.35 }}>{update.title}</h3>
            </div>
            <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', flexShrink: 0 }}>
              {formatRelativeTime(update.createdAt)}
            </span>
          </div>
          <p style={{ fontSize: '0.9375rem', lineHeight: 1.75, color: 'var(--text-secondary)', whiteSpace: 'pre-wrap' }}>{update.content}</p>
          
          {update.media && update.media.length > 0 && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 12, marginTop: 16 }}>
              {update.media.map((m: any) => (
                <div key={m._id} style={{ borderRadius: 'var(--radius-md)', overflow: 'hidden', border: '1px solid var(--border-subtle)', background: 'var(--bg-elevated)', aspectRatio: '16/9' }}>
                  {m.type === 'image' && (
                    <img src={m.url} alt="attached media" style={{ width: '100%', height: '100%', objectFit: 'cover' }} loading="lazy" />
                  )}
                  {m.type === 'video' && (
                    <video src={m.url} controls style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                  )}
                  {m.type === 'document' && (
                    <a href={m.url} target="_blank" rel="noopener noreferrer" style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textDecoration: 'none', padding: 8 }}>
                      <FileText size={24} style={{ color: 'var(--indigo-400)', marginBottom: 8, flexShrink: 0 }} />
                      <span title={m.originalName || 'Document'} style={{ fontSize: '0.75rem', color: 'var(--text-primary)', fontWeight: 600, textAlign: 'center', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden', wordBreak: 'break-all' }}>
                        {m.originalName || 'Doc'}
                      </span>
                    </a>
                  )}
                </div>
              ))}
            </div>
          )}

          <div style={{ marginTop: 14, paddingTop: 14, borderTop: '1px solid var(--border-subtle)', display: 'flex', alignItems: 'center', gap: 8 }}>
            <div className="avatar avatar-sm" style={{ background: author?.avatarColor || 'var(--indigo-700)' }}>
              {author?.avatarInitials}
            </div>
            <span style={{ fontSize: '0.8125rem', color: 'var(--text-muted)' }}>
              Posted by <span style={{ fontWeight: 600, color: 'var(--text-secondary)' }}>@{author?.username}</span>
            </span>

            {isMod && !update.isApproved && (
              <button 
                onClick={onApprove}
                className="btn btn-sm btn-primary" 
                style={{ marginLeft: 'auto' }}
              >
                Approve Update
              </button>
            )}
          </div>
        </div>
      </div>
    </div>
  );
}

function CommentItem({ comment, onMediaClick }: { comment: any; onMediaClick?: (media: any) => void }) {
  const user = comment.userId;

  return (
    <div className="comment">
      <div className="avatar" style={{ background: user?.avatarColor || 'var(--indigo-700)' }}>
        {user?.avatarInitials || '??'}
      </div>
      <div className="comment-body">
        <div className="comment-header">
          <span className="comment-author">@{user?.username || 'unknown'}</span>
          <span className="comment-time">{formatRelativeTime(comment.createdAt)}</span>
        </div>
        <p className="comment-text">{comment.content}</p>
        {comment.media && comment.media.length > 0 && (
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginTop: 12 }}>
            {comment.media.map((m: any, i: number) => (
              <div 
                key={i} 
                onClick={() => m.type !== 'document' && onMediaClick?.(m)}
                style={{ 
                  width: 120, height: 80, borderRadius: 'var(--radius-md)', 
                  background: 'var(--bg-elevated)', overflow: 'hidden', border: '1px solid var(--border-subtle)',
                  position: 'relative', cursor: m.type === 'document' ? 'default' : 'pointer'
                }}>
                {m.type === 'image' && <img src={m.url} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />}
                {m.type === 'video' && (
                  <>
                    <video src={m.url} style={{ width: '100%', height: '100%', objectFit: 'cover', pointerEvents: 'none' }} />
                    <div style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center'}}>
                      <span style={{background: 'rgba(0,0,0,0.6)', color: 'white', padding: '4px 8px', borderRadius: 4, fontSize: '0.65rem'}}>Video</span>
                    </div>
                  </>
                )}
                {m.type === 'document' && (
                  <a href={m.url} target="_blank" rel="noopener noreferrer" style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textDecoration: 'none' }}>
                    <FileText size={20} style={{ color: 'var(--indigo-400)', marginBottom: 4 }} />
                    <span style={{ fontSize: '0.65rem', color: 'var(--text-primary)', fontWeight: 600, textAlign: 'center', padding: '0 4px', wordBreak: 'break-all', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                      {m.originalName || 'Document'}
                    </span>
                  </a>
                )}
              </div>
            ))}
          </div>
        )}
      </div>
    </div>
  );
}
